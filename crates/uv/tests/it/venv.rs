...
fn relocatable_real_environment() {
    ...
    // The `home` in pyvenv.cfg points to the environment's own scripts directory.
    let scripts_path = scripts.path().simplified_display().to_string();
    pyvenv_cfg.assert(predicates::str::contains(format!("home = {scripts_path}")));
